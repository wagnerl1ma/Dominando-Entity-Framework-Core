using DominandoEFCore_Modulo15_Dicas.Data;
using DominandoEFCore_Modulo15_Dicas.Domain;
using Microsoft.EntityFrameworkCore;

namespace DominandoEFCore_Modulo15_Dicas
{
    internal class Program
    {
        ...
        static void ConsultaFiltrada()
        {
            ...
            var sql = db.Departamentos.Include(p => p.Colaboradores.Where(c => c.Nome.Contains("Teste"))).ToQueryString();
        ...
        static void Clear()
        {
            ...
            db.ChangeTracker.Clear(); //limpa os objetos que estavam sendo rastreados e redefine o estado do contexto
        }
        ...
        static void ToQueryString()
        {
            ...
            var sql = query.ToQueryString(); // envia Instrucao Sql que será executada, mas nao executa
        ...
        static void SingleOrDefaultVsFirstOrDefault()
        {
            ...
            _ = db.Departamentos.SingleOrDefault(p => p.Id > 2); //se tiver valores repetidos irá estourar exceçao
            ...
            var sql = db.Departamentos
                ...
                .Select(p =>
                    new
                    {
                        Descricao = p.Key,
                        Contador = p.Count(),
                        Media = p.Average(p => p.Id), //Calculo de média
                        Maximo = p.Max(p => p.Id),
                        Soma = p.Sum(p => p.Id)
                    }).ToQueryString();
